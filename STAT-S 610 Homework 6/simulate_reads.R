...
make_true_seqs = function(n_snps, n_true_seqs, n_nonzero = NULL) {
  true_seqs = matrix(NA, nrow = n_true_seqs, ncol = n_snps)
  for(i in 1:n_snps) {
    if(!is.null(n_nonzero)) {
      # The specified number of nonzero entries
      snp_vals = rep(0, n_true_seqs)
      nonzero_indices = sample(n_true_seqs, n_nonzero)
      snp_vals[nonzero_indices] = 1
    } else{
